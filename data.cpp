...
void read_MNIST_raw(string filename, string labelname, uchar *images,
                    unsigned *labels, const unsigned batch) {
  ...
  memset(images, 0,
         batch * image_height * image_width * image_channel * sizeof(uchar));
...
void read_MNIST_normalized(string filename, string labelname, float *images,
                           unsigned *labels, const unsigned batch) {
  const int image_height = 28;
  const int image_width = 28;
  const int image_channel = 1;
  ...
  memset(images, 0,
         batch * image_height * image_width * image_channel * sizeof(float));
  memset(labels, 0, batch * sizeof(unsigned));
    ...
    for (int i = 0; i < batch; ++i)
      for (int h = 0; h < image_height; ++h)
        for (int w = 0; w < image_width; ++w) {
          uchar temp = 0;
          file.read((char *)&temp, sizeof(temp));
          images[i * image_height * image_width + h * image_width + w] =
              (((float)temp / 255.f) - 0.1307f) / 0.3081f; // Normalize
        }
  } else {
    fprintf(stderr, "Error in reading MNIST image file '%s' in data.cpp.\n",
            filename.c_str());
...
//========================== CIFAR-10 ================================
void read_CIFAR10_raw(string filename, unsigned *images, unsigned *labels,
    ...
    for (int l = 0; l < batch; ++l) {
      ...
      for (int c = 0; c < image_channel; c++) // channel
        for (int h = 0; h < image_height; h++)
          for (int w = 0; w < image_width; w++) {
            file.read((char *)&temp, sizeof(temp));
            ui[(l * image_height * image_width + h * image_width + w) * 4 + c] =
                temp;
          }
    }
  } else {
...
void read_CIFAR10_normalized(string filename, float *images, unsigned *labels,
                             const unsigned batch) {
  const int image_height = 32;
  const int image_width = 32;
  const int image_channel = 3;
...
//========================== ImageNet =================================
void read_raw_jpeg(const char *filename, uchar *fdata, int image_height,
                   int image_width) {
  unsigned long x, y;
  ...
  for (int c = 0; c < 3; c++) // channel
  {
    for (int h = 0; h < image_height; h++) {
      for (int w = 0; w < image_width; w++) {
        ...
        fdata[(h * image_width + w) * 4 + c] =
            jdata[c * x * y + map_h * x + map_w];
      }
    }
  }
  free(jdata);
...
void read_ImageNet_raw(const char *descfilename, unsigned *images,
                       unsigned *labels, int batch) {
  ...
  for (int i = 0; i < batch; i++) {
    ...
    labels[i] = tag;
    read_raw_jpeg(filename, (uchar *)&images[i * image_height * image_width],
...
inline float blerp(float c00, float c10, float c01, float c11, float tx,
                   float ty) {
  return lerp(lerp(c00, c10, tx), lerp(c01, c11, tx), ty);
}
void scale_transform(const uchar *src, uchar *dst, int dst_h, int dst_w,
                     int src_h, int src_w) {
  int x, y;
...
void read_normalized_jpeg(const char *filename, float *fdata, int image_height,
                          int image_width) {
  ...
  while (info.output_scanline < info.output_height) {
    ...
    rowptr[0] =
        (uchar *)hwc_data + 3 * info.output_width * info.output_scanline;
    jpeg_read_scanlines(&info, rowptr, 1);
  }
  jpeg_finish_decompress(&info);
  ...
  if (org_width < org_height) {
    scaled_width = 256;
    scaled_height = int(scale * org_height);
  } else {
    scaled_height = 256;
    scaled_width = int(scale * org_width);
  }

  // printf("=========%d,%d=========\n",scaled_height, scaled_width);
  uchar *scale_data =
      (uchar *)malloc(3 * scaled_height * scaled_width * sizeof(uchar));
  scale_transform(chw_data, scale_data, scaled_height, scaled_width, org_height,
                  org_width);
  ...
  for (int h = 0; h < image_height; h++) {
    for (int w = 0; w < image_width; w++) {
      ...
      // printf("h:%d, w:%d, map_h:%d, map_w:%d, scale:%f, scaled_height:%d,
      // scaled_width:%d\n", h,w,map_h,map_w,scale, scaled_height,
      // scaled_width);

      fdata[0 * image_height * image_width + h * image_width + w] =
          ((float)scale_data[0 * scaled_width * scaled_height +
                             map_h * scaled_width + map_w] /
           255.0f) *
              4.3668f -
          2.1179f;
...
void read_ImageNet_normalized(const char *descfilename, float *images,
                              unsigned *labels, int batch) {
  const int image_height = 224;
  const int image_width = 224;
  const int image_channel = 3;
  ...
  for (int i = 0; i < batch; i++) {
    ...
    read_normalized_jpeg(
        filename, &images[i * image_height * image_width * image_channel],
        image_height, image_width);
  }
