...
#PBS -V
#PBS -l select=8:ncpus=4:ngpus=4
#PBS -r y
...
batch=2048
# Log dir path
log_path="/home/tartarughina/TCBNN/benn_log"
...
# Execute 5 times
for i in {1..5}; do
    mpiexec --envall --np "${NTOTRANKS}" --ppn "${nranks}" --hostfile "$PBS_NODEFILE" --cpu-bind list:0,8,16,24 ./benn_nccl -b "${batch}" -u -t > "${log_path}/UMT_gpus_${NTOTRANKS}_batch_${batch}_iter_${i}.txt"
done
