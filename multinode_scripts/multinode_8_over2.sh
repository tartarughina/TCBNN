...
# Execute 5 times
for i in {1..5}; do
    mpiexec --envall --np "${NTOTRANKS}" --ppn "${nranks}" --hostfile "$PBS_NODEFILE" --cpu-bind list:0,8,16,24 ./benn_nccl -b "${batch}" -u --oversub 2 > "${log_path}/OVER2_gpus_${NTOTRANKS}_batch_${batch}_iter_${i}.txt"
done
