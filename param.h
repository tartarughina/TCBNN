...
__global__ void PackFcWeight128(const float *__restrict__ A, uin32 *B,
  ...
  for (int bid = blockIdx.x; bid < gdx * gdy; bid += gridDim.x) {
    ...
    float f0 =
        ((bx * 128 + lx * 32 + laneid < A_height) && (by * 8 + ly < A_width))
            ? A[(bx * 128 + lx * 32 + laneid) * A_width + by * 8 + ly]
            : -1.0f;
    unsigned r0 = __brev(__ballot_sync(0xFFFFFFFF, f0 >= 0 ? 1 : 0));
    if (laneid == 0)
      B[(by * 8 + ly) * gdx * 4 + bx * 4 + lx] = r0;
  }
}
...
__global__ void UnPackFcOutput128(const uin32 *__restrict__ A, float *B,
  ...
  for (int bid = blockIdx.x; bid < gdx * gdy; bid += gridDim.x) {
    ...
    if ((bx * 8 + lx < A_height) && (by * 128 + ly * 32 + laneid < A_width))
      B[(bx * 8 + lx) * A_width + by * 128 + ly * 32 + laneid] =
          2 * (float)((r0 >> (31 - laneid)) & 0x1) - 1;
  }
}
...
public:
  In128LayerParam(const char *_name, int _input_height, int _input_width,
                  bool unified_mem = false, bool um_tuning = false)
      : input_height(_input_height), output_height(_input_height),
        input_width(_input_width), output_width(_input_width), input(NULL),
        input_gpu(NULL), output(NULL), output_gpu(NULL),
        unified_mem(unified_mem), um_tuning(um_tuning) {
  ...
  In128LayerParam *initialize(float *input) {
    ...
    if (unified_mem) {
      input_gpu = input;
      SAFE_ALOC_UM(output, output_bit_bytes());
      ...
      CUDA_SAFE_CALL(
          cudaMemcpy(input_gpu, input, input_bytes(), cudaMemcpyHostToDevice));
  ...
  In128LayerParam *ready() {
    ...
    if (unified_mem) {
      ...
      CUDA_SAFE_CALL(cudaMemcpy(this->gpu, this, sizeof(In128LayerParam),
                                cudaMemcpyHostToDevice));
      return this->gpu;
    }
  }
  void set_output_gpu(uin32 *_output_gpu) { this->output_gpu = _output_gpu; }
  ...
  float *download_full_output() {
    ...
    if (unified_mem) {
      SAFE_ALOC_UM(full_output, size);
      full_output_gpu = full_output;
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &numBlocksPerSm, UnPackFcOutput128FMT, numThreads, 0);
    UnPackFcOutput128FMT<<<numBlocksPerSm * deviceProp.multiProcessorCount,
                           numThreads>>>(output_gpu, full_output_gpu,
                                         output_height, output_width);
#else
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &numBlocksPerSm, UnPackFcOutput128, numThreads, 0);
    UnPackFcOutput128<<<numBlocksPerSm * deviceProp.multiProcessorCount,
                        numThreads>>>(output_gpu, full_output_gpu,
                                      output_height, output_width);
#endif
...
public:
  Fc128LayerParam(const char *name, int _input_height, int _input_width,
                  int _weight_width, bool unified_mem = false,
      ...
      : weight_height(_input_width), weight_width(_weight_width),
        ...
        bn_width(_weight_width), weight(NULL), weight_gpu(NULL), bn(NULL),
        bn_gpu(NULL), output(NULL), output_gpu(NULL), input(NULL),
        input_gpu(NULL), gpu(NULL), unified_mem(unified_mem),
        um_tuning(um_tuning) {
  ...
  Fc128LayerParam *ready() {
    ...
    if (unified_mem) {
      ...
      CUDA_SAFE_CALL(cudaMemcpy(this->gpu, this, sizeof(Fc128LayerParam),
                                cudaMemcpyHostToDevice));
      return this->gpu;
    }
  }
  void set_input_gpu(uin32 *_input_gpu) { this->input_gpu = _input_gpu; }
  Fc128LayerParam *initialize(FILE *config_file, uin32 *prev_layer_gpu) {
    // Process weight
    if (unified_mem) {
      SAFE_ALOC_UM(weight, weight_bytes());
      SAFE_ALOC_UM(weight_gpu, weight_bit_bytes());
...
#ifdef NEWFMT
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &numBlocksPerSm, PackFcWeight128FMT, numThreads, 0);
    PackFcWeight128FMT<<<numBlocksPerSm * deviceProp.multiProcessorCount,
                         numThreads>>>(weight_float, weight_gpu, weight_height,
                                       weight_width);
#else
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &numBlocksPerSm, PackFcWeight128, numThreads, 0);
    PackFcWeight128<<<numBlocksPerSm * deviceProp.multiProcessorCount,
                      numThreads>>>(weight_float, weight_gpu, weight_height,
                                    weight_width);
#endif
    CUDA_CHECK_KERNEL();
    ...
    if (!unified_mem) {
      ...
      CUDA_SAFE_CALL(
          cudaMemcpy(bn_gpu, bn, bn_bytes(), cudaMemcpyHostToDevice));
    }
...
public:
  Out128LayerParam(const char *name, int _input_height, int _input_width,
                   int _weight_width, bool unified_mem = false,
                   bool um_tuning = false)
      : input_height(_input_height), input_width(_input_width),
        weight_height(_input_width), weight_width(_weight_width),
        output_height(_input_height), output_width(_weight_width), input(NULL),
        input_gpu(NULL), output(NULL), output_gpu(NULL), weight(NULL),
        weight_gpu(NULL), unified_mem(unified_mem), um_tuning(um_tuning) {
    strncpy(this->name, name, 8);
  }
  // row major
  int input_size() { return input_height * input_width; }
  ...
  Out128LayerParam *ready() {
    ...
    if (unified_mem) {
      ...
      CUDA_SAFE_CALL(cudaMemcpy(this->gpu, this, sizeof(Out128LayerParam),
                                cudaMemcpyHostToDevice));
      return this->gpu;
    }
  }
  ...
  Out128LayerParam *initialize(FILE *config_file, uin32 *prev_layer_gpu) {
    ...
    if (!unified_mem) {
      CUDA_SAFE_CALL(cudaMemcpy(bn_scale_gpu, bn_scale, bn_bytes(),
                                cudaMemcpyHostToDevice));
      CUDA_SAFE_CALL(
          cudaMemcpy(bn_bias_gpu, bn_bias, bn_bytes(), cudaMemcpyHostToDevice));
    }
  ...
  void release() {
    if (unified_mem) {
      SAFE_FREE_UM(weight);
      SAFE_FREE_UM(output);
      SAFE_FREE_UM(bn_scale);
      SAFE_FREE_UM(bn_bias);
    } else {
      SAFE_FREE_HOST(weight);
      SAFE_FREE_HOST(output);
...
__global__ void PackFiltersByInChannels128(const float *__restrict__ filter,
  ...
  const int ins =
      4 * STEP128(input_channels); // condense C:in_channel into 32bit-unsigned

  for (int c = 0; c < ins; c++) // iter over C:input_channels
  {
    // From shape[filter_height, filter_width, input_channels, output_channels]
    float f0 = ((c * 32 + laneid) < input_channels)
                   ? filter[bx * input_channels * output_channels +
                            (c * 32 + laneid) * output_channels + by]
                   : -1.0f;
    unsigned r0 = __brev(__ballot_sync(0xFFFFFFFF, f0 >= 0));
    if (laneid == 0) // avoid warp conflict
      // To shape[filter_height, filter_width, output_channels,
      // input_channels/32]
...
__global__ void PackFiltersByOutChannels32(const float* __restrict__ filter,
        unsigned* filter_binarized, const int in_channels, const int
out_channels, const int filter_width, const int filter_height)
{
    unsigned laneid; asm("mov.u32 %0, %%laneid;":"=r"(laneid));
    const int bx = blockIdx.x;//iter over (filter_width*filter_height)
    const int by = blockIdx.y;//iter over input_channels
    const int ots = STEP32(out_channels);//condense K:output_channel into
32bit-unsigned

    for (int k=0; k<ots*4; k++) //iter over K:output_channels
    {
        // From shape[filter_height, filter_width, in_channels, out_channels]
        float f0 = ((k*32+laneid)<out_channels)?
filter[bx*in_channels*out_channels
...
__global__ void PackFiltersByOutChannels32(const float *__restrict__ filter,
  ...
  const int by = blockIdx.y; // iter over input_channels
  const int ots =
      STEP32(out_channels); // condense K:output_channel into 32bit-unsigned
...
__global__ void UnpackConvOutput32(const unsigned *__restrict__ input_binarized,
                                   float *input, const int input_height,
  ...
  /*const int ins = STEP32(input_channels);//condense C:in_channel into
   * 32bit-unsigned*/
  const int ins =
      STEP128(input_channels); // condense C:in_channel into 32bit-unsigned
  ...
  for (int c = 0; c < ins * 4; c++) // iter over C:in_channels
  {
    ...
    unsigned r0 =
        input_binarized[by * input_width * PAD8(batch) * ins * 4 +
                        bx * PAD8(batch) * ins * 4 + bz * ins * 4 + c];
    // To shape[batch, input_height, input_width, in_channels]
    if (c * 32 + laneid < input_channels) {
      input[bz * input_height * input_width * input_channels +
            by * input_width * input_channels + bx * input_channels + c * 32 +
            laneid] = 2 * (float)((r0 >> (31 - laneid)) & 0x1) - 1;
    }
  }
}

__global__ void
UnpackConvOutput32FMT(const unsigned *__restrict__ input_binarized,
                      float *input, const int input_height,
  ...
  {
    // From shape[input_height, input_width, batch, in_channels/32]
    // unsigned r0 = input_binarized[by*input_width*PAD8(batch)*ins*4 +
    // bx*PAD8(batch)*ins*4
    //    + bz*ins*4 + c];
    unsigned r0 =
        input_binarized[by * input_width * PAD8(batch) * ins * 4 +
...
public:
  InConv128LayerParam(const char *name, int _input_height, int _input_width,
                      int _filter_height, int _filter_width,
                      int _input_channels, int _output_channels, int _batch,
                      int _stride_height = 1, int _stride_width = 1,
                      bool _padding = true, int _pool_height = 1,
      ...
      : input_height(_input_height), input_width(_input_width),
        ...
        batch(_batch), stride_height(_stride_height),
        stride_width(_stride_width), pool_height(_pool_height),
        pool_width(_pool_width), save_residual(_save_residual),
        padding(_padding), bn(NULL), filter(NULL), output(NULL),
        output_gpu(NULL), input(NULL), input_gpu(NULL), gpu(NULL),
        output_residual_gpu(NULL), unified_mem(unified_mem),
        um_tuning(um_tuning)

  {
    strncpy(this->name, name, 8);
    this->pad_h = padding ? ((((input_height + stride_height -
                                (input_height % stride_height)) /
                                   stride_height -
                               1) *
                                  stride_height +
    ...
    this->pad_w =
        padding
            ? ((((input_width + stride_width - (input_width % stride_width)) /
                     stride_width -
                 1) *
                    stride_width +
    ...
    output_height =
        (buf_height + pool_height - 1) / pool_height;         // pooling height
  ...
  InConv128LayerParam *ready() {
    ...
    if (!unified_mem) {
      ...
      CUDA_SAFE_CALL(cudaMemcpy(this->gpu, this, sizeof(InConv128LayerParam),
                                cudaMemcpyHostToDevice));
      return this->gpu;
    }
  ...
  InConv128LayerParam *initialize(float *input, FILE *config_file) {
    ...
    if (unified_mem) {
      input_gpu = input;
      SAFE_ALOC_UM(filter, filter_bytes());
    ...
    if (unified_mem) {
      filter_float = (float *)filter;
      SAFE_ALOC_UM(filter_gpu, filter_bit_bytes());
      ...
      CUDA_SAFE_CALL(cudaMemcpy(filter_float, filter, filter_bytes(),
                                cudaMemcpyHostToDevice));
    }
    ...
    // Binarize Filter
    PackFiltersByOutChannels32<<<
        dim3(filter_height * filter_width, input_channels), 32>>>(
        filter_float, filter_gpu, input_channels, output_channels, filter_width,
        filter_height);
    if (!unified_mem)
    ...
    if (!unified_mem)
      CUDA_SAFE_CALL(cudaMemcpy(this->bn_gpu, this->bn, bn_bytes(),
                                cudaMemcpyHostToDevice));
  ...
  float *download_full_output() {
    ...
    float *full_output_gpu = NULL;
    if (unified_mem) {
      SAFE_ALOC_UM(full_output, output_bytes());
      full_output_gpu = full_output;
    ...
    UnpackConvOutput32FMT<<<dim3(output_width, output_height, batch), 32>>>(
        ...
        output_channels, batch);
#else
    UnpackConvOutput32<<<dim3(output_width, output_height, batch), 32>>>(
        output_gpu, full_output_gpu, output_height, output_width,
        output_channels, batch);
#endif
    if (!unified_mem) {
      CUDA_SAFE_CALL(cudaMemcpy(full_output, full_output_gpu, output_bytes(),
                                cudaMemcpyDeviceToHost));
...
public:
  Conv128LayerParam(const char *name, int _input_height, int _input_width,
                    ...
                    int _stride_width = 1, bool _padding = true,
                    int _pool_height = 1, int _pool_width = 1,
                    bool _ahead_fc = false, bool _save_residual = false,
                    bool _inject_residual = false, int _residual_channels = 0,
                    bool _residual_pool = false, bool unified_mem = false,
                    bool um_tuning = false)
      :
        ...
        batch(_batch), stride_height(_stride_height),
        stride_width(_stride_width), pool_height(_pool_height),
        pool_width(_pool_width), ahead_fc(_ahead_fc),
        save_residual(_save_residual), inject_residual(_inject_residual),
        residual_channels(_residual_channels), padding(_padding),
        residual_pool(_residual_pool), bn(NULL), bn_gpu(NULL), filter(NULL),
        filter_gpu(NULL), output(NULL), output_gpu(NULL), input(NULL),
        input_gpu(NULL), gpu(NULL), output_residual_gpu(NULL),
        input_residual_gpu(NULL), unified_mem(unified_mem),
        um_tuning(um_tuning) {
    strncpy(this->name, name, 8);
    this->pad_h = padding ? ((((input_height + stride_height -
                                (input_height % stride_height)) /
  ...
  Conv128LayerParam *ready() {
    ...
    CUDA_SAFE_CALL(cudaMemcpy(this->gpu, this, sizeof(Conv128LayerParam),
                              cudaMemcpyHostToDevice));
    return this->gpu;
  }
  void set_input_gpu(uin32 *input_gpu) { this->input_gpu = input_gpu; }
  ...
  Conv128LayerParam *initialize(FILE *config_file, uin32 *prev_layer_gpu,
...
#ifdef NEWFMT
    PackFiltersByInChannels128FMT<<<
        dim3(filter_height * filter_width, output_channels), 32>>>(
        filter_float, filter_gpu, input_channels, output_channels, filter_width,
        filter_height);
#else
    PackFiltersByInChannels128<<<
        dim3(filter_height * filter_width, output_channels), 32>>>(
        filter_float, filter_gpu, input_channels, output_channels, filter_width,
        filter_height);
#endif
    ...
    // inject residual
    if (inject_residual)
      set_input_residual_gpu(input_residual_gpu);
    return this->ready();
  }
  uin32 *get_output_gpu() { return this->output_gpu; }
  int *get_output_residual_gpu() { return this->output_residual_gpu; }
