...
#define CHECK_NULL_POINTER(X)                                                  \
  __checkNullPointer(__FILE__, __LINE__, (void **)&(X))
...
#define SAFE_ALOC_UM(X, Y)                                                     \
  CUDA_SAFE_CALL(cudaMallocManaged((void **)&(X), (Y)));
// CPU host free
...
// Unified Memory free (same as GPU device free)
#define SAFE_FREE_UM(X) SAFE_FREE_GPU(X)
 ...
 * (X)->name, clock64()-t0);*/
#define TICK_KERNEL_TIMER(X)                                                   \
  if (threadIdx.x == 0 && blockIdx.x == 0)                                     \
    printf("Layer-%s takes %lld cycles.\n", (X)->name, clock64() - t0);
...
// validation
void validate_prediction(float *prediction, unsigned *labels,
                         const unsigned categories, const unsigned batch) {
  printf("======Label======\n");
...
__device__ __inline__ void store128(const void *addr, T a, T b, T c, T d) {
  *((float4 *)addr) = make_float4(*(float *)(&a), *(float *)(&b),
                                  *(float *)(&c), *(float *)(&d));
}
